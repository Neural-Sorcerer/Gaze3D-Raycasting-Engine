"""
MediaPipe Face Mesh 3D Template

This module provides the 3D coordinates for the MediaPipe Face Mesh model's 468 landmarks.
The coordinates represent a standardized face model in 3D space, with the origin positioned
inside the face.

The coordinate system follows the convention:
- X-axis: Positive values toward the right side of the face
- Y-axis: Positive values toward the top of the face
- Z-axis: Positive values toward the front of the face

These coordinates can be used for:
1. Head pose estimation via PnP algorithm
2. 3D visualization of facial landmarks
3. Mapping between 2D detections and 3D face structure
"""

import numpy as np

try:
    from mediapipe.python.solutions import face_mesh_connections as mpc
except ImportError:
    mpc = None


# -------------------------------
# 1. Define the MediaPipe Face Model Template and Face Coordinate System (468, 3)
# -------------------------------
face_mesh_template_3D_cm: np.ndarray = np.array(
    [
    ...
    dtype=np.float32,
) # in centimeters (approximate; may actually be "unitless")

face_mesh_template_3D_cm -= face_mesh_template_3D_cm[1]     # Nose origin
face_mesh_template_3D_cm *= np.array([1, 1, 1])             # Flip: X, Y, Z axis
face_mesh_template_3D_mm = face_mesh_template_3D_cm * 10.0  # Scale to mm (✅)
face_mesh_template_3D_dm = face_mesh_template_3D_cm / 10.0  # Scale to dm
face_mesh_template_3D_m = face_mesh_template_3D_cm / 100.0  # Scale to m

# Define key points for head pose estimation
# These points are stable and work well for PnP algorithm
POSE_LANDMARKS = {
    "nose_tip": 1,              # Nose tip
    "chin": 152,                # Chin
    "left_eye_outer": 263,      # Left eye outer corner
    "right_eye_outer": 33,      # Right eye outer corner
    "left_mouth_outer": 287,    # Left mouth corner
    "right_mouth_outer": 57     # Right mouth corner
}

# Create a subset of landmarks commonly used for head pose estimation
face_model_pose = face_mesh_template_3D_mm[[
    POSE_LANDMARKS["nose_tip"],
    POSE_LANDMARKS["chin"],
    POSE_LANDMARKS["left_eye_outer"],
    POSE_LANDMARKS["right_eye_outer"],
    POSE_LANDMARKS["left_mouth_outer"],
    POSE_LANDMARKS["right_mouth_outer"]
]]


# -------------------------------
# 2. Define 3D Face Model Pose correspond to standard facial landmarks in 3D space (6, 3)
# -------------------------------
face_model_pose_ChatGPT = np.array([
...
def connections2landmarks(edges):
    return np.array(sorted({i for edge in edges for i in edge}))


# Create a subset of landmarks
if mpc is None:
    face_mesh_subset = {}
else:
    face_mesh_subset = {
        "CONTOURS":         connections2landmarks(mpc.FACEMESH_CONTOURS),
        "FACE_OVAL":        connections2landmarks(mpc.FACEMESH_FACE_OVAL),
        "IRISES":           connections2landmarks(mpc.FACEMESH_IRISES),
        "LEFT_EYE":         connections2landmarks(mpc.FACEMESH_LEFT_EYE),
        "LEFT_EYEBROW":     connections2landmarks(mpc.FACEMESH_LEFT_EYEBROW),
        "LEFT_IRIS":        connections2landmarks(mpc.FACEMESH_LEFT_IRIS),
        "LIPS":             connections2landmarks(mpc.FACEMESH_LIPS),
        "NOSE":             connections2landmarks(mpc.FACEMESH_NOSE),
        "RIGHT_EYE":        connections2landmarks(mpc.FACEMESH_RIGHT_EYE),
        "RIGHT_EYEBROW":    connections2landmarks(mpc.FACEMESH_RIGHT_EYEBROW),
        "RIGHT_IRIS":       connections2landmarks(mpc.FACEMESH_RIGHT_IRIS),
        "TESSELATION":      connections2landmarks(mpc.FACEMESH_TESSELATION),
    }


def save_face_model(path="face_mesh_MediaPipe_template.npz"):
    """
    Save the face model to a NumPy .npz file for faster loading in the future.
    
    Args:
        path (str): Path to save the face model
    """
    np.savez(
        path,
        face_mesh_template_3D=face_mesh_template_3D_mm, # All landmarks
        face_model_pose=face_model_pose,                # Head pose landmarks
    )
    
    print(f"Face model saved to {path}")


def load_face_model(path="face_mesh_MediaPipe_template.npz"):
    """
    Load a previously saved face model from a NumPy .npz file.
    
    Args:
        path (str): Path to the saved face model
    
    Returns:
        dict: Dictionary containing the face model data
    """
...
if __name__ == "__main__":
    # Example usage
    print(f"Face mesh template contains {len(face_mesh_template_3D_mm)} landmarks")
    print(f"Key points for head pose estimation:\n{face_model_pose}")
    
    # Save and load
    save_face_model("face_mesh_MediaPipe_template.npz")
    data = load_face_model("face_mesh_MediaPipe_template.npz")
    print(data["face_model_pose"])
